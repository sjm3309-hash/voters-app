...
const TOP_LEAGUES = [
  { leagueId: 39, name: "EPL" },
  { leagueId: 140, name: "LaLiga" },
  { leagueId: 135, name: "SerieA" },
  { leagueId: 78, name: "Bundesliga" },
  { leagueId: 61, name: "Ligue1" },
] as const;
...
async function fetchLeague(apiKey: string, leagueId: number, season: number) {
...
export async function POST() {
  try {
    ...
    const all = await Promise.allSettled(
      TOP_LEAGUES.map((l) => fetchLeague(apiKey, l.leagueId, season).then((rows) => ({ l, rows }))),
    );
    ...
    for (const r of all) {
      ...
      for (const row of r.value.rows) {
        ...
        const confirmedAt = Number.isFinite(kickoffMs)
          ...
          : null;
        const home = row.teams.home.name?.trim() ?? "";
        const away = row.teams.away.name?.trim() ?? "";
        upsertRows.push({
          external_id: `football:${r.value.l.leagueId}:${row.fixture.id}`,
          title: `${home} vs ${away}`,
          ...
          category: "스포츠",
          sub_category: "해외축구",
          league_id: String(row.league?.id ?? r.value.l.leagueId),
          status: "active" as const,
    ...
    if (errors.length > 0) {
      return NextResponse.json({ ok: false, errors, season }, { status: 502 });
    }
